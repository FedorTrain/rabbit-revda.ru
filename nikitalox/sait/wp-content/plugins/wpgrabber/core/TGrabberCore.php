    ...
    */
    function getContent($url)
    {
        ...
        if (!$this->requestMethod) { // CURL
            ...
            if ($this->config->get('requestTime')) curl_setopt($ch, CURLOPT_TIMEOUT, $this->config->get('requestTime'));
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            if ($this->config->get('curlRedirectOn')) curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
    ...
    */
    function getImageUrl($url)
    {
        if (substr_count($url, 'http://') or substr_count($url, 'https://')) {
			...
			if (substr_count($url, '//')) return $url='http:' . $url;
    ...
    */
    private function _import()
    {
        ...
        elseif ($this->feed['type'] == 'vk') { // vk
            ...
            preg_match_all('|<div class="post_date"><a  class="post_link"  href="/(wall-\d+_\d+)".*?<div class="wall_text">(.*?)<div class="post_full_like_wrap clear_fix">|is', $index, $matches);
            if (!count($matches)) {
            ...
            foreach ($links as $link) {
                ...
                $this->content[$link]['text'] = preg_replace('|<span[^>]+style="display:\s*none[^>]+>|is', '<span>', $this->content[$link]['text']);
                $this->content[$link]['text'] = preg_replace('~<div[^>]+class="(?:media_desc|page_media_link|wall_marked|feedback|page_gif|page_post_queue_wide|wall_signed|page_media_poll).*?</div>~is', '', $this->content[$link]['text']);
              /*  $this->content[$link]['text'] = preg_replace('|<div class="page_post_queue_wide">.*?</div>|is', '', $this->content[$link]['text']);
				$this->content[$link]['text'] = preg_replace('|<div[^>]+class="page_gif_.*?</div>|is', '', $this->content[$link]['text']);
				$this->content[$link]['text'] = preg_replace('|<div class="feedback.*?</div>|is', '', $this->content[$link]['text']);
                $this->content[$link]['text'] = preg_replace('|<div class="wall_signed">.*?</div>|is', '', $this->content[$link]['text']); */
                $this->content[$link]['text'] = preg_replace('|<div[^>]+background-image:\s*url\(([^\)]+)[^>]+>|is', '<img src="$1" />', $this->content[$link]['text']);
				$this->content[$link]['text'] = preg_replace('|<img[^>]+src="(/images/emoji/)|is', '<img src="https://vk.com$1', $this->content[$link]['text']);
                $this->content[$link]['text'] = preg_replace('|<a[^>]+onclick="return showPhoto.*?base&quot;:&quot;(.*?)&quot;,&quot;x_&quot;:\[&quot;(.*?)&quot;.*?>|is', '<img src="$1$2.jpg" />', $this->content[$link]['text']);
				$this->content[$link]['text'] = preg_replace('|<a[^>]+href=".*?" onclick="return showPhoto.*?base:&quot;(.*?)&quot;,x_:\[&quot;(.*?)&quot;.*?>|is', '<img src="$1$2.jpg" />', $this->content[$link]['text']);
    ...
    */
    function imageParser($matches) 
    {
        ...
        if ($this->feed['params']['image_save']) { // saving images on the server
			//$imageFile = $this->getMDNameFile(basename($matches[3])); //оригинальная строка
			
			
			if (strstr ($matches[3], '.jpg') == '.jpg' or strstr ($matches[3], '.png') == '.png' or strstr ($matches[3], '.gif') == '.gif' or strstr ($matches[3], '.jpeg') == '.jpeg'){
				$imageFile = $this->getMDNameFile(basename($matches[3]));}
			else{
				$imageFile = $this->getMDNameFile(basename($matches[3])).'.jpg';}
            ...
            if ($this->copyUrlFile($matches[3], $imageFile)) {
                ...
                $this->_echo(' - <a target="_blank" href="' . $matches[3] . '" style="color:green; font-weight: bold">OK</a>');
                // resizing
...
*/        
        ...
        if ($this->imagesContentNoSave) {
            $this->_echo("<br>Материал не будет сохранен по причине отсутсвия в нем картинок! (см. опцию: Не сохранять материал без картинок)");
            return null;
        }
        ...
        if (empty($record['text'])) {
          $this->_echo('<br /><i>Материал не будет сохранен по причине отсутствия в нем контента</i>');
          return null;
        }
        return true;
    }
