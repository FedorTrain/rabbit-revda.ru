...
Plugin URI: http://wpworld.ru/dagon-design-sitemap-generator/
Description: Плагин генерирует карту сайта. Скачано с <a href="http://wpworld.ru" target="_blank">Мир Wordpress</a> - статьи, плагины, темы.
Author: Dagon Design
Version: 3.12
...
function ddsg_options_page() {
	...
	<p>Для получения информации перейдите на страницу плагина на сайте <a href="http://wpworld.ru/dagon-design-sitemap-generator/">Мир Wordpress</a></p>
...
//	exit();
	...
	while (($cats[$k]->category_parent != $parent) && ($k < $num_cats)) {
		$k++;
	}
...
function ddsg_get_pages($page_data, $pages, $num_pages, $pages_with_children, $excluded_pages, $comments_on_pages, $show_page_date, $parent = 0, $level = 0) {
	...
	$k = 0;
	while (($pages[$k]->post_parent != $parent) && ($k < $num_pages)) {
		$k++;
	}
	while (($pages[$k]->post_parent == $parent) && ($k < $num_pages)) {
...
function ddsg_merge_cats_posts($post_data, $posts, $cat_data, $num_posts, $num_cats, $comments_on_posts, $show_post_date) {
	...
	for ($c = 0; $c < $num_cats; $c++) {
		...
		$k = 0;
		while (($posts[$k]->category_id != $cat_data[$c]['cat_id']) && ($k < $num_posts)) {
			$k++;
		}
...
function ddsg_create_sitemap() {
	...
	$t_out .= '<p align="right" style="font-size:90%;">' . DDSG_CREDITS . ' <a href="http://wpworld.ru" title="Сайт о Wordpress">Мир Wordpress</a></p>';
